...
- [x] V0 通过；两领域确实使用同一配置/文本入口，没有增加独立插件或版本平台。
...
- [x] V0、VCLI 通过；公共模型保留两领域语义，旧 loader 不再构成第二份业务权威。
...
- [x] V0 通过；共识差异能从配置解释，alias 退出不依靠静默忽略输入实现。
...
- [x] V0 通过；主流程和辅助入口无遗漏的 Health 回退或错误路径，未扩大评估/日志范围。
...
- [x] V0 通过；UI 不止切换标签，表单、操作、审核身份和结果真正一致。
...
- [x] AC01：现有工作和统一改造均进入远端 main。
- [x] AC02：所有已发现边界有唯一 owner 或具名保留理由。
- [x] AC03：减少重复/无用抽象，没有为凑行数或未来领域制造新复杂度。
- [x] AC04：两领域完整流程与单/多产品结果通过验证。
- [x] AC05：UI 领域、能力、确认、队列、结果隔离正确。
- [x] AC06：无 aliases 新功能与运行依赖，旧信息只读可追踪。
- [x] AC07：必要旧格式/批准契约可用，无静默覆盖或降级验证。
- [x] AC08：现有周边能力保留；第三领域未加入，各层验证限制如实记录。

完成任务时追加实际提交与验证结果再勾选，不以历史测试代替本轮证据：

| 任务 | 提交/实际变更 | 验证结果及限制 | B/AC |
| --- | --- | --- | --- |
| T01–T02 基线审计 | 远端核实：main 24f31ee，Travel 012fdfe，43 提交；独立基线审查 | 初始 371 tests；修复批处理失败退出码、Health 平铺字段与旧标签精确对齐后 373 tests（372 pass / 1 DB skip），compileall 和两个主 CLI help 通过 | B19/B20 |
...
收尾验证（2026-09-10，实现提交 `a00a904`）：

| 任务 | 实际结果与证据 |
| --- | --- |
| T08–T09 | 共享 discovery/compiler/extractor；inline 请求契约禁止 legacy 降级；单/多产品、跨领域校验和失败路径离线通过。默认输出遵循 manifest，显式路径拒绝覆盖，同名来源可区分。 |
| T10–T12 | aliases 资源和新提案退出；投票规则、保护字段来自 manifest；queue/decisions/base 身份匹配，resume 重算审核结果。历史 alias 操作与无身份旧队列不静默应用。 |
| T13–T15 | 可信分类驱动分析；Travel 无标签指标为 N/A；历史 CLI ExtractionResult 可读。stability/cost 使用所选 manifest，Canonical 候选复用批准映射，批准文件与 origin/main 字节一致。 |
| T16–T18 | 全局领域/能力操作列表；切换清空表单、确认和结果；修改命令参数使确认失效。审核控件绑定 queue，Canonical 批准绑定源内容和输出路径。旧 prompt 导出、Travel validator/builder wrapper 和 adapter ID 过渡层已删除。 |
| T19 | `388 tests`: 387 pass / 1 PostgreSQL live skip；compileall、diff --check、7 个 CLI help 通过。`test_multi_vertical_flow` 使用真实编排，仅替换 PDF/model 边界，覆盖两领域发现→共识→审核→抽取→分析以及错误身份/输出保护。 |

浏览器：本地 Chrome 实测 Health→Travel 切换、能力变化、参数确认失效、缺失 schema 的编译错误与结果归属；临时队列 Accept→Apply 成功且 pending 未应用；Canonical 候选预览不批准，修改输出后确认/审核人清空。控制台未捕获 error/warn；320px 截图与 768/1024/1440px 布局检查通过。所有浏览器写入仅在 `/private/tmp/capstone-browser-fixtures`，无模型/DB 调用。

审查：独立审查发现的普通抽取覆盖、compiler 默认 validator 丢失 manifest、stability 丢失 manifest 均已修复并加入回归；最终独立复核两次遇到 usage 限额终止，未记为通过。主代理完成调用链/最终差异复核。没有新增依赖、第三领域、数据表或管理平台。
