...
class LLMExtractor:
    ...
    def extract(self, extraction_input: ExtractionInput) -> ExtractionResult:
        ...
        if not self.allow_fallback:
            raise RuntimeError(
                f"{self.provider}/{self.model} extraction is unavailable and fallback is disabled."
            )
    ...
    def _build_system_prompt(self, schema: VerticalSchema) -> str:
        if self._is_discovered_json_schema():
            return (
                f"You are extracting a single flat JSON record from an Australian "
                f"{schema.vertical} insurance product PDF.\n"
                "Return exactly the discovered schema field names at the top level; "
                "do not create hospital/extras wrapper objects.\n"
                "FIELD DEFINITIONS:\n"
                f"{self.loader.build_field_definitions(schema)}\n\n"
                "VALID ENUM VALUES:\n"
                f"{self.loader.build_enum_constraints(schema)}\n\n"
                "RULES:\n"
                "- First determine product_type from the document and path context.\n"
                "- Populate only fields whose applies_to includes that product_type.\n"
                "- Set non-applicable fields and absent fields to null.\n"
                "- Add _unfilled with field names you could not populate from this PDF.\n"
                "- Use _notes for ambiguity, otherwise null.\n"
    ...
    def _build_user_prompt(self, extraction_input: ExtractionInput) -> str:
        text = self._build_relevant_text(extraction_input)
        if extraction_input.mode == "table_assisted":
            tables = json.dumps(
                self._select_relevant_tables(extraction_input.tables),
                ensure_ascii=False,
            )
            return f"FULL TEXT:\n{text}\n\nTABLES:\n{tables}"
        return f"FULL TEXT:\n{text}"

    def _build_relevant_text(self, extraction_input: ExtractionInput) -> str:
        pages = extraction_input.source_document.text_by_page or [extraction_input.text]
        sections: list[str] = []
        seen: set[int] = set()

        for page_index, page_text in enumerate(pages[:2], start=1):
            if page_text.strip():
                sections.append(f"[Page {page_index} opening]\n{page_text.strip()[:5000]}")
                seen.add(page_index)
        ...
        alias_terms = self._schema_alias_terms()
        scored_pages: list[tuple[int, int, str]] = []
        for page_index, page_text in enumerate(pages, start=1):
            lowered = page_text.lower()
            score = sum(2 for keyword in keywords if keyword in lowered)
            score += sum(1 for alias in alias_terms if alias and alias in lowered)
            if score:
                scored_pages.append((score, page_index, page_text))

        for _score, page_index, page_text in sorted(scored_pages, key=lambda item: (-item[0], item[1])):
            if page_index in seen:
                continue
            sections.append(f"[Page {page_index} relevant]\n{page_text.strip()[:5000]}")
            seen.add(page_index)
            if sum(len(section) for section in sections) >= 28000:
                break

        if not sections:
            return extraction_input.text[:28000]
        return "\n\n".join(sections)[:32000]

    def _select_relevant_tables(self, tables: list[list[list[str]]]) -> list[list[list[str]]]:
        if not tables:
            return []
        alias_terms = self._schema_alias_terms()
        keywords = (
        ...
        for index, table in enumerate(tables):
            text = " ".join(cell for row in table for cell in row if cell).lower()
            score = sum(2 for keyword in keywords if keyword in text)
            score += sum(1 for alias in alias_terms if alias and alias in text)
            if score:
                scored.append((score, index, table))
        selected = [table for _score, _index, table in sorted(scored, key=lambda item: (-item[0], item[1]))[:16]]
        return selected or tables[:8]

    def _schema_alias_terms(self) -> list[str]:
        terms: set[str] = set()
        for section in self.schema.active_sections().values():
            for canonical in section.canonical_names:
                terms.add(self._normalize_text(self._decamelize(canonical)).strip())
            for aliases in section.aliases.values():
                for alias in aliases:
                    terms.add(self._normalize_text(alias).strip())
        return sorted(terms)
    ...
    def _heuristic_extract(self, extraction_input: ExtractionInput) -> ExtractionResult:
        if self._is_discovered_json_schema():
            ...
            record = self._to_discovered_json_record(
                extraction_input,
                data,
            )
            return ExtractionResult(
                ...
                source_path=extraction_input.source_document.path,
                provider="heuristic",
                model="rule-based-private-health-discovered-json",
                data=record,
                evidences=evidences,
    ...
    def _extract_private_health(
        ...
        lines, line_pages = self._lines_with_pages(extraction_input)
        hospital_data, hospital_evidence = self._extract_hospital(lines, line_pages)
        extras_data, extras_evidence, normalizations = self._extract_extras(
            lines,
            extraction_input.tables,
            line_pages,
        )
        data: dict[str, Any] = {}
    ...
    ) -> tuple[dict[str, Any], dict[str, list[Evidence]]]:
        ...
        for index, line in enumerate(lines):
            ...
            for category in matched_categories:
                ...
                evidence_map[f"hospital.clinical_categories.{category}"].append(
                    Evidence(text=line, page=self._page_for_line(index, line_pages))
                )
    ...
    def _extract_extras(
        ...
        service_pages: dict[str, list[int | None]] = defaultdict(list)
        service_table_values = self._extract_extras_from_tables(tables or [], alias_map)
        for service, table_data in service_table_values.items():
            service_blocks[service].extend(table_data["rows"])
            service_pages[service].extend([None] * len(table_data["rows"]))
        ...
        for index, line in enumerate(lines):
            ...
            if matched:
                ...
                service_pages[current_service].append(self._page_for_line(index, line_pages))
                continue
            if current_service and not self._looks_like_extras_boundary(line):
                service_blocks[current_service].append(line)
                service_pages[current_service].append(self._page_for_line(index, line_pages))
            elif self._looks_like_extras_boundary(line):
        ...
        for service in ordered_services:
            ...
            explicit_shared = self._extract_combined_services(joined, alias_map, service)
            if pending_combined_service and covered:
                shared_with = [pending_combined_service]
                pending_combined_service = None
            elif explicit_shared:
                shared_with = explicit_shared
            elif has_combined_limit and covered:
                pending_combined_service = service

            if block:
                evidence_map[f"extras.services.{service}"].append(
                    Evidence(text=block[0], page=self._first_page(service_pages.get(service, [])))
                )
    ...
    def _extract_limit_values(self, text: str) -> dict[str, float | None]:
        ...
        per_person = self._extract_contextual_amount(
            text,
            r"(?:per\s+person|per\s+patient|pp\b|individual)",
        )
        per_policy = self._extract_contextual_amount(
            text,
            r"(?:per\s+policy|per\s+membership|per\s+family|membership)",
        )
        annual = self._extract_contextual_amount(
            text,
            r"(?:annual\s+limit|yearly\s+limit|per\s+calendar\s+year)",
        )
        if per_person is not None:
            result["per_person"] = per_person
        if per_policy is not None:
            result["per_policy"] = per_policy
        elif annual is not None and "person" not in lowered:
            result["per_policy"] = annual
        if result["per_person"] is not None or result["per_policy"] is not None:
            return result
    ...
    def _extract_extras_from_tables(
        self,
        ...
        for table in tables:
            for row in table:
                cells = [self._clean_line(cell) for cell in row if self._clean_line(cell)]
                if not cells:
                    continue
                joined = " | ".join(cells)
                matched = self._find_canonical_matches(joined, alias_map)
                if not matched:
                    continue
                for service in matched:
                    entry = table_values.setdefault(service, {"rows": []})
                    entry["rows"].append(joined)
        return table_values

    def _extract_combined_services(
        self,
        text: str,
        alias_map: dict[str, list[str]],
        current_service: str,
    ) -> list[str]:
        if "combined" not in text.lower():
            return []
        services = [
            service
            for service in self._find_canonical_matches(text, alias_map)
            if service != current_service
        ]
        return sorted(set(services))

    def _looks_like_extras_boundary(self, line: str) -> bool:
        lowered = line.lower()
        if lowered.startswith(("important", "note", "terms", "conditions", "how to claim")):
            return True
        if lowered in {"hospital", "extras", "general treatment", "general health", "ambulance"}:
            return True
        if self._looks_like_heading(line) and not re.search(
            r"\b(waiting|limit|covered|included|benefit|dental|optical|physio|therapy)\b",
            lowered,
        ):
            return True
        return False
    ...
    @staticmethod
    def _extract_contextual_amount(text: str, context_pattern: str) -> float | None:
        amount_pattern = r"\$\s*[\d,]+(?:\.\d{2})?"
        patterns = (
            rf"({amount_pattern})[^\n|]{{0,40}}{context_pattern}",
            rf"{context_pattern}[^\n|]{{0,40}}({amount_pattern})",
        )
        for pattern in patterns:
            match = re.search(pattern, text, flags=re.IGNORECASE)
            if match:
                return LLMExtractor._parse_amount(match.group(1))
        return None
    ...
    def _lines_with_pages(self, extraction_input: ExtractionInput) -> tuple[list[str], list[int | None]]:
        if extraction_input.source_document.text_by_page:
            lines: list[str] = []
            pages: list[int | None] = []
            for page_number, page_text in enumerate(extraction_input.source_document.text_by_page, start=1):
                for line in page_text.splitlines():
                    lines.append(self._clean_line(line))
                    pages.append(page_number)
            return lines, pages
        lines = [self._clean_line(line) for line in extraction_input.text.splitlines()]
    ...
    @staticmethod
    def _page_for_line(index: int, line_pages: list[int | None] | None) -> int | None:
        if line_pages is None or index >= len(line_pages):
            return None
        return line_pages[index]

    @staticmethod
    def _first_page(pages: list[int | None]) -> int | None:
        for page in pages:
            if page is not None:
                return page
        return None
    ...
    def _is_discovered_json_schema(self) -> bool:
        return self.schema.metadata.get("schema_style") == "discovered_json"

    def _to_discovered_json_record(
        self,
        extraction_input: ExtractionInput,
        legacy_data: dict[str, Any],
    ) -> dict[str, Any]:
        source_schema = self.schema.metadata.get("source_schema", {})
        fields = source_schema.get("fields", []) if isinstance(source_schema, dict) else []
        field_names = [
            str(field["name"])
            for field in fields
            if isinstance(field, dict) and field.get("name")
        ]
        product_type = self._infer_discovered_product_type(
            extraction_input.source_document.path,
            bool(legacy_data.get("hospital")),
            bool(legacy_data.get("extras")),
        )
        product_name = (
            legacy_data.get("hospital", {}).get("product_name")
            or legacy_data.get("extras", {}).get("product_name")
            or self._extract_product_name(
                [self._clean_line(line) for line in extraction_input.text.splitlines()]
            )
        )
        ...
        if "hospital_tier" in record:
            record["hospital_tier"] = legacy_data.get("hospital", {}).get("hospital_tier")
        record["_unfilled"] = [
    ...
    @staticmethod
    def _infer_discovered_product_type(
        source_path: str,
        has_hospital: bool,
        has_extras: bool,
    ) -> str:
        lowered_parts = [part.lower() for part in Path(source_path).parts]
        if "combined" in lowered_parts:
            return "combined"
        if "hospital" in lowered_parts:
            return "hospital"
        if "extras" in lowered_parts:
            return "extras"
        if "generalhealth" in lowered_parts:
