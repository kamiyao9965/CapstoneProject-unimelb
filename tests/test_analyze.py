...
class ProductApplicabilityTest(unittest.TestCase):
    ...
    def test_unclassified_model_result_still_uses_source_category_denominator(self) -> None:
        ...
        self.assertEqual(
            analysis.product_type_mismatches,
            {"hospital -> unclassified": 1},
        )

    def test_reports_source_category_coverage_and_classification_accuracy(self) -> None:
        analysis = analyze(
            [
                ExtractionRecord(
                    data={"product_type": "hospital"},
                    source_document="data/A/hospital/one.pdf",
                    source_category="hospital",
                ),
                ExtractionRecord(
                    data={"product_type": "extras"},
                    source_document="data/B/combined/two.pdf",
                    source_category="combined",
                ),
            ],
            [FieldSpec("product_type")],
        )

        self.assertEqual(
            analysis.source_category_counts,
            {"combined": 1, "hospital": 1},
        )
        self.assertEqual(analysis.product_type_correct, 1)
        self.assertEqual(analysis.product_type_accuracy, 0.5)

    def test_feedback_reports_product_type_mismatches_separately(self) -> None:
        analysis = analyze(
            [
                ExtractionRecord(
                    data={"product_type": "extras"},
        ...
        feedback = build_feedback(analysis)
        data = build_feedback_data(analysis)["analysis"]

        self.assertIn("directory category", feedback)
        self.assertIn("hospital -> extras", feedback)
        self.assertEqual(data["source_category_counts"], {"hospital": 1})
        self.assertEqual(data["product_type_correct"], 0)
        self.assertEqual(data["product_type_unclassified"], 0)
        self.assertEqual(data["product_type_accuracy"], 0.0)
        self.assertEqual(
            data["product_type_mismatches"],
            {"hospital -> extras": 1},
        )
    ...
    def test_report_renders_zero_denominator_as_not_applicable(self) -> None:
        ...
        self.assertIn("Product type classification: 100% (1/1 correct", output.getvalue())
